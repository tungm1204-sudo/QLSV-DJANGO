...
| Thiết kế Model từ "Technical Schema" thay vì từ "User Story" — dẫn đến thiếu field thực tế | Chỉ copy field từ DBML sơ khai sang Model mà không tự hỏi "Người dùng thực tế cần làm gì với bảng này?". Ví dụ: `Semester` thiếu `registration_start/end`, `tuition_deadline`; `Room` thiếu `floor`; `Department.manager` chỉ trỏ Lecturer trong khi Trưởng phòng hành chính là Staff. | **BẮT BUỘC**: Với MỌI Model mới, trước khi viết code phải tự đặt và trả lời 3 câu hỏi: (1) *"Module nào SAU NÀY sẽ đọc/ghi bảng này?"* (2) *"Người dùng cuối cần nhìn thấy thông tin gì từ bảng này?"* (3) *"Có field nào chỉ đúng với 1 loại entity nhưng đang được dùng chung cho nhiều loại không?"*. Nếu DBML ban đầu quá sơ khai so với thực tế, BẮT BUỘC đề xuất bổ sung và hỏi Tech Lead trước khi code. |
| Sinh code boilerplate tự động (script) rồi không review lại — dẫn đến Selector/API thiếu filter | Dùng script tự động sinh Selector/Service/View cho các model mới trong `master_data`. Script chỉ tạo ra cấu trúc tối thiểu (`filter is_active`), không có filter theo FK (`?building_id`, `?major_id`), không có `?search=`. Sau đó không quay lại review xem Frontend cần gì. | **BẮT BUỘC**: Sau khi dùng bất kỳ script/boilerplate nào để sinh code, phải đọc lại từng Selector và tự hỏi: *"Frontend sẽ cần filter/search bảng này theo tiêu chí gì khi render Dropdown hay Table?"*. Nếu có FK trên Model, Selector BẮT BUỘC phải hỗ trợ filter theo FK đó. |
| Đặt `null=True` trên ForeignKey để tránh lỗi migration ngắn hạn, tạo ra lỗi nghiệp vụ dài hạn | `Room.building` được đặt `null=True` để migration không bị lỗi khi chưa có dữ liệu Building. Nhưng về nghiệp vụ, một phòng học KHÔNG THỂ không thuộc tòa nhà nào — đây là dữ liệu bắt buộc. | **BẮT BUỘC**: Khi cân nhắc đặt `null=True` cho một ForeignKey, phải tự hỏi: *"Trong thực tế nghiệp vụ, record này CÓ THỂ tồn tại mà không có liên kết đến bảng kia không?"*. Nếu câu trả lời là KHÔNG → xóa `null=True`. Giải pháp đúng là tạo seed data cho bảng phụ trước (VD: tạo Campus/Building mặc định khi setup hệ thống), không phải nới lỏng constraint ở Model. |
| Đẩy việc hoàn thiện logic Backend sang cho Frontend (Dev B) gánh vác | Hài lòng với các API "cơ bản" (chỉ có CRUD) mà không có Filter nâng cao, Search, Validation chặt chẽ, khiến Frontend không thể làm UI và phải vòng lại sửa code Backend. | **BẮT BUỘC**: Backend phải là người chịu trách nhiệm 100% về tính sẵn sàng của dữ liệu. Frontend chỉ đóng vai trò hiển thị (Render). Trước khi bàn giao API, Backend bắt buộc phải tự đóng vai trò Frontend để hỏi: *"Mình có đủ API để vẽ cái Select Box này chưa? Dữ liệu trả về có bị N+1 query không? Tìm kiếm thế nào?"*. Không được để Frontend phải đi "dọn rác" cho Backend. |
| Test thiếu tính thực tế | Chỉ test bằng Python/Script (của backend) mà bỏ qua Frontend (nếu có). Dẫn đến code Backend chạy đúng nhưng lên UI bị lỗi (không click được, thiếu Toast, lệch API). | **BẮT BUỘC**: Nếu chức năng ĐÃ CÓ Frontend, bắt buộc phải khởi chạy Server và dùng trình duyệt (Browser Subagent) để test trực tiếp trên UI giống hệt người dùng. Nếu chưa có Frontend thì mới test chay bằng script/API. |
| Suy đoán cấu trúc dự án dẫn đến làm việc thừa | Đề xuất tạo mới một app (vd: `financials`) chỉ dựa trên phân tích DBML mà không chịu kiểm tra xem trong source code thực tế đã có sẵn app tương đương (vd: `finance`) hay chưa. | **BẮT BUỘC**: Trước khi đề xuất tạo thư mục, tạo app mới hay file mới, LUÔN LUÔN dùng tool `list_dir` hoặc tương đương để kiểm tra cấu trúc thư mục thực tế của dự án. Không bao giờ được phép "nhắm mắt đoán bừa" rồi bắt tay vào làm. |
