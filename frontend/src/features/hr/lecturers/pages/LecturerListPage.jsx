    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="bg-white" onClick={() => fileInputRef.current?.click()} disabled={importMutation.isPending}>
            <FileUp size={16} className="mr-2" /> 
            {importMutation.isPending ? 'Đang tải...' : 'Import'}
          </Button>
          <input type="file" ref={fileInputRef} className="hidden" accept=".xlsx,.xls" onChange={handleImport} />
          
          <Button variant="outline" className="bg-white" onClick={handleExport} disabled={isExporting}>
          ...
          <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => navigate('/hr/lecturers/new')}>
            <Plus size={16} className="mr-2" /> Thêm Giảng viên
          </Button>
        </div>
      </div>
