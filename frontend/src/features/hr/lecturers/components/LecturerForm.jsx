...
import { lecturerSchema } from '../validations/lecturerSchema';
...
import { 
  ...
  useDegreeOptions,
  useAcademicTitleOptions,
  useEthnicityOptions,
  useReligionOptions,
  useNationalityOptions
} from '../../../master_data/hooks/useMasterDataOptions';

export default function LecturerForm({ initialData, onSubmit, onCancel, isLoading }) {
  const form = useForm({
    resolver: zodResolver(lecturerSchema),
    defaultValues: initialData || {
      email: '', password: '', full_name: '',
      lecturer_code: '', department: null, degree: null,
      academic_title: null, contract_type: 'FULL_TIME', teaching_domain: '',
  ...
  const { data: academicTitles = [] } = useAcademicTitleOptions();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <section className="bg-slate-50/50 p-6 rounded-xl border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-2">
            <Label htmlFor="lecturer_code" className="after:content-['*'] after:ml-0.5 after:text-red-500">Mã giảng viên</Label>
            <Input id="lecturer_code" {...register('lecturer_code')} className={errors.lecturer_code ? "border-red-500" : ""} />
            {errors.lecturer_code && <p className="text-sm text-red-500">{errors.lecturer_code.message}</p>}
          </div>
          
          {renderSelect('department', 'Đơn vị / Khoa / Bộ môn', departments)}
          {renderSelect('degree', 'Học vị', degrees)}
          {renderSelect('academic_title', 'Học hàm', academicTitles)}
          
          <div className="space-y-2">
            <Label htmlFor="contract_type" className="after:content-['*'] after:ml-0.5 after:text-red-500">Loại hợp đồng</Label>
            <Select value={watch('contract_type')} onValueChange={(val) => setValue('contract_type', val, { shouldValidate: true })}>
              <SelectTrigger id="contract_type" className={errors.contract_type ? "border-red-500" : ""}>
                <SelectValue placeholder="Chọn loại hợp đồng">
                  {
                    watch('contract_type') === 'FULL_TIME' ? 'Cơ hữu' :
                    watch('contract_type') === 'VISITING' ? 'Thỉnh giảng' :
                    watch('contract_type') === 'GUEST' ? 'Khách mời' : undefined
                  }
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="teaching_domain">Lĩnh vực giảng dạy (Mô tả)</Label>
            <Input id="teaching_domain" {...register('teaching_domain')} />
          </div>
